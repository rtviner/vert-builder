class DayGenerator
  ...
  def build_recovery_distribution(week)
    ...
    remaining_5 = if remaining_5_days_total / 5.0 >= 90
      build_recovery_normal_remaining(remaining_5_days_total, week_vert)
    else
  ...
  def build_progression_days(week)
    ...
    easy_days = if easy_days_total >= 4 * 90
      max_easy = [ week_vert * 0.15, easy_days_total - (90 * 3) ].min
      randomize_days_with_sum(total: easy_days_total, count: 4, min: 90, max: max_easy)
    else
  ...
  def build_goal_days(week, goal_vertical_distance)
    ...
    remaining_days = if remaining_4_days_total / 4.0 >= 90
  ...
  def build_recovery_normal_remaining(total, week_vert)
    ...
    remaining = randomize_days_with_sum(total: remainder, count: 4, min: 0, max: week_vert * 0.15)
    [ one_day ] + remaining
  end
  ...
  def build_goal_normal_remaining(total, recovery_vert)
    one_day = [ 90, 100 ].sample
    remainder = total - one_day
    remaining = randomize_days_with_sum(total: remainder, count: 3, min: 0, max: recovery_vert * 0.15)
    [ one_day ] + remaining
  end
